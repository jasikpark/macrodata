...
**`state/flags.md`** - How things reach the user across sessions. One line + pointer per flag.
- Flags only reach the user when said out loud — silent state file edits terminate in the model's context
- `## 🔴 Unresolved` section: items that MUST be surfaced at session start

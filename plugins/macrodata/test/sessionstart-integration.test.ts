...
describe("SessionStart integration", () => {
  ...
  afterEach(() => {
    ...
    // session-start also starts a recall worker, which has no pidfile — find it
    // the way the hook does, by the sentinel + root in its argv. Scoped to this
    // test's root, so it can never reach a worker serving a real store.
    try {

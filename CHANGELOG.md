...
- [#84](https://github.com/jasikpark/macrodata/pull/84) [`edf7856`](https://github.com/jasikpark/macrodata/commit/edf785624adffd87ae4eadf901df737fc8b693a1) Thanks [@jasikpark](https://github.com/jasikpark)! - Conversation indexing no longer rewrites the whole index once per exchange. Incremental updates commit every 128 exchanges and a full rebuild commits once. This keeps the PreCompact and SessionEnd hooks from holding a session for minutes when a long transcript changes: on an 84 MB index, a pass that took 62.7 s now takes 3.4 s. A pass that dies midway re-indexes only its uncommitted batch next time, instead of every file it had already finished.
...
- [#67](https://github.com/jasikpark/macrodata/pull/67) [`272a6c4`](https://github.com/jasikpark/macrodata/commit/272a6c4c52cfab1ec3bf76693d24d6ad0dcaee5c) Thanks [@jasikpark](https://github.com/jasikpark)! - Recall worker hardening, following the hook-managed worker that shipped in 0.9.0.

  The pidfile-claim guard now asks the same question as the `ps` classifier: is this
  process a worker _for this state root_? It used to accept any process carrying the
  ...
  the hook now reaps the _installed_ workers on that root. Every worker drains the same
...
- [#69](https://github.com/jasikpark/macrodata/pull/69) [`9e44589`](https://github.com/jasikpark/macrodata/commit/9e4458900da6cc47b5efb8eb2e07fbbb626b7d27) Thanks [@jasikpark](https://github.com/jasikpark)! - Throttle the tools-hint hook to fire every 11 turns instead of every turn.
